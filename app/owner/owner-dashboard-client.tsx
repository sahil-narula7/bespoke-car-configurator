...
export default function OwnerDashboardClient() {
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-2xl border border-[#d9c6a1]/18 bg-[#101a29] panel-glow">
      <div className="border-b border-[#d9c6a1]/16 px-5 py-4 text-sm text-[#d6cab9]">
      ...
      {!loading && !error && submissions.length === 0 ? (
        <div className="px-5 py-10 text-sm text-[#d6cab9]">No submissions yet.</div>
      ) : null}

      {!loading && !error && submissions.length > 0 ? (
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-[#d9c6a1]/12 text-left text-sm text-[#efe4d1]">
            <thead className="bg-[#142234] text-xs uppercase tracking-[0.12em] text-[#d0b07a]">
              <tr>
            ...
            <tbody className="divide-y divide-[#d9c6a1]/12">
              {submissions.map((submission, index) => (
                <tr key={`${submission.email}-${submission.submittedAt}-${index}`}>
                  <td className="px-4 py-3 text-[#cdbca5]">{formatDate(submission.submittedAt)}</td>
                  ...
                  <td className="max-w-sm px-4 py-3 text-[#d6cab9]">{submission.message || "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
